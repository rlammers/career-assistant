...
- The retained revision uses both expected digest-qualified images. Its sole credential-free registry entry uses the expected user-assigned identity, which retains one registry-scoped `AcrPull` assignment; system events confirm both images were pulled without an image-pull failure.
...
| SQLite on Azure Files | Controlled clean-database recovery failed; not accepted for continued use | Reassess the persistence design before another deployment attempt. Do not weaken locking or resume runtime verification on the current mount. |
| Azure identity and service exposure | Foundation and workload image-pull identity verified; external ingress disabled after failed recovery | Foundation least-privilege identity, registry controls, observability integration, storage linkage, immutable revision references, and both identity-authenticated pulls are verified. Revalidate ingress isolation after a replacement persistence design is deployed. |
...
| Private application deployment | Incremental deployment succeeded; one controlled clean-database recovery failed, after which the sole revision was stopped and external ingress disabled | Azure resource creation verified; runtime behavior blocked |
